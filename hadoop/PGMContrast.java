...
public class PGMContrast
{
        ...
        throws IOException, ImageFormatException
    {
        ...
        for(int i = n-2; i >= 0; i--) {
            if(i == 0) g[i] = PGMTransform.applyAdd(PGMTransform.applyQuantize(g[i], 3.0, 1.5), PGMTransform.applyExpand(g[i+1]));
            else if(i == 1) g[i] = PGMTransform.applyAdd(PGMTransform.applyQuantize(g[i], 3.0, 1.5), PGMTransform.applyExpand(g[i+1]));
            else if(i == 2) g[i] = PGMTransform.applyAdd(PGMTransform.applyQuantize(g[i], 2.0, 1.2), PGMTransform.applyExpand(g[i+1]));
            else if(i == 3) g[i] = PGMTransform.applyAdd(PGMTransform.applyQuantize(g[i], 1.8, 0.8), PGMTransform.applyExpand(g[i+1]));
            else if(i == 4) g[i] = PGMTransform.applyAdd(PGMTransform.applyQuantize(g[i], 1.6, 0.5), PGMTransform.applyExpand(g[i+1]));
	    }
        return g[0];
}
