...
public class ImageRecordReader 
        	implements RecordReader<IntWritable, ImageWritable>{
    ...
    public ImageRecordReader(JobConf conf, ImageSplit split) {
        ...
        try {
            FileSystem fileSys = 
    	        FileInputFormat.getInputPaths(conf)[0].getFileSystem(conf);
	    inputStream = 
    	        fileSys.open(FileInputFormat.getInputPaths(conf)[0]);
        } catch (IOException e) {
    ...
    @Override
    public boolean next(IntWritable key, ImageWritable value) 
    						throws IOException {
        if (finished == false) {
        	...
        	System.out.println("IOException in next() of " + 
    							"ImageRecordReader");
            }
